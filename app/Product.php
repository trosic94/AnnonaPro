...
class Product extends Model
{
    ...
    public static function productDATA_bySLUG($productSLUG,$catSLUG)
    {
        ...
        $productDATA = DB::table('products as PROD')
                            ...
                            ->select(
                                ...
                                'PROD.attr_weight as prod_attr_weight',
                                'PROD.attr_quantity as prod_attr_quantity',
                                'CAT.parent_id as cat_parent_id',
                                'CAT.name as cat_name',
                                'CAT.slug as cat_slug',
                                'CAT.cat_image as cat_image',
    ...
    public static function productsBy_SpecialOptions($specOption)
    {
        $allProducts = DB::table('products as PROD')
                            ->join('categories as CAT','PROD.category_id','CAT.id')
                            ->leftJoin('categories as PCAT','CAT.parent_id','PCAT.id')
                            ...
                            ->where('PROD.status',1)
                            ->where('PROD.product_price','!=',0)
                            ->where('SOP.special_options_id',$specOption)
                            ->select(
                                'PROD.id as prod_id',
                                'PROD.sku as prod_sku',
                                'PROD.title as prod_title',
    ...
    public static function productsBy_CAT_IDs_ALL($catIDs)
    {
        $allProducts = DB::table('products as PROD')
                            ->join('categories as CAT','PROD.category_id','CAT.id')
                            ->leftJoin('categories as PCAT','CAT.parent_id','PCAT.id')
    ...
    public static function productsBy_Tags($tagIDs,$excludeProdID)
    {
        $allProducts = DB::table('products as PROD')
                            ->join('categories as CAT','PROD.category_id','CAT.id')
                            ->leftJoin('categories as PCAT','CAT.parent_id','PCAT.id')
                            ->leftJoin('manufacturer as M','PROD.manufacturer_id','M.id')
                            ->join('product_tags as PT','PT.product_id','PROD.id')
                            ->leftJoin('special_options_products as SOP','SOP.product_id','PROD.id')
                            ->leftJoin('badges_products as BP','BP.product_id','PROD.id')
                            ->leftJoin('badges as B','B.id','BP.badge_id')
                            ->where('PROD.status',1)
                            ->where('PROD.product_price','!=',0)
                            ->where('PROD.id','!=',$excludeProdID)
