...
class V_CategoryController extends VoyagerBaseController
{
    ...
    public function store(Request $request)
    {
        ...
        if ($useLOY_DATA && $useLOY_DATA->active == 1 && $shopCAT == $data->parent_id):

            // pripremam podatke za EXPORT
            $exportDATA = array();
            $exportDATA['apikey'] = $useLOY_DATA->api_key;

            $exportDATA['stavke'][0]['p_sifrakat_partnera'] = $data->id;
            $exportDATA['stavke'][0]['p_naziv'] = $data->name;

            $catExport = AppSpotlight::catExport($useLOY_DATA->api_base_url,$exportDATA);
