<div class="specTABs">
  ...
  <ul class="nav nav-tabs nav-justified md-tabs blue ml-0 mr-0 p-0 rounded-0 z-depth-0" id="tabsSpecial" role="tablist">
    @foreach ($specialOptions_tabs as $key => $tab)
    <li class="nav-item">
      <a class="nav-link {{ ($tab->id == 1)? 'active':'' }} rounded-0" orderVAL="{{ $tab->id }}" id="spec_{{ $tab->id }}-tab-just" data-toggle="tab" href="#spec_{{ $tab->id }}-just" role="tab" aria-controls="spec_{{ $tab->id }}-just" aria-selected="true">
      	{{ $tab->title }}
      </a>
    </li>
    @endforeach
  ...
  @foreach ($specialOptions_tabs as $key => $tab)
    <div class="tab-pane fade {{ ($tab->id == 1)? 'show active':'' }}" id="spec_{{ $tab->id }}-just" role="tabpanel" aria-labelledby="spec_{{ $tab->id }}-tab-just">
