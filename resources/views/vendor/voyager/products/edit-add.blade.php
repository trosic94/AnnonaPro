                                    ...
                                    @foreach($productCategories_SEL as $catSEL)
                                        ...
                                        @if (count($catSEL['cat_sub']) > 0)
                                            ...
                                            @foreach($catSEL['cat_sub'] as $catSUB_sel)
                                                ...
                                                <option value="{{ $catSUB_sel['cat_id'] }}" {{ ($catSUB_sel['cat_published'] == '0')? 'disabled':'' }} {{ ($edit && $dataTypeContent->category_id == $catSUB_sel['cat_id'])? 'selected':'' }}>- {{ $catSUB_sel['cat_name'] }}</option>

                                                @if (count($catSUB_sel['cat_subsub']) > 0)

                                                    @foreach($catSUB_sel['cat_subsub'] as $catSUBSUB_sel)

                                                        <option value="{{ $catSUBSUB_sel['cat_id'] }}" {{ ($catSUBSUB_sel['cat_published'] == '0')? 'disabled':'' }} {{ ($edit && $dataTypeContent->category_id == $catSUBSUB_sel['cat_id'])? 'selected':'' }}>- - {{ $catSUBSUB_sel['cat_name'] }}</option>
