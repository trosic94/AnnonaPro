                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <select class="form-control select2 select2-hidden-accessible" name="category_id[]" multiple required>
                                        ...
                                        @foreach($productCategories_SEL as $catSEL)
                                            ...
                                            <option value="{{ $catSEL['cat_id'] }}" {{ ($catSEL['cat_published'] == '0')? 'disabled':'' }} {{ (in_array($catSEL['cat_id'], $categoriesForAttribute))? 'selected':'' }}>{{ $catSEL['cat_name'] }}</option>

                                            @if (count($catSEL['cat_sub']) > 0)

                                                @foreach($catSEL['cat_sub'] as $catSUB_sel)

                                                    <option value="{{ $catSUB_sel['cat_id'] }}" {{ ($catSUB_sel['cat_published'] == '0')? 'disabled':'' }} {{ (in_array($catSUB_sel['cat_id'], $categoriesForAttribute))? 'selected':'' }}>- {{ $catSUB_sel['cat_name'] }}</option>

                                                    @if (count($catSUB_sel['cat_subsub']) > 0)

                                                        @foreach($catSUB_sel['cat_subsub'] as $catSUBSUB_sel)

                                                            <option value="{{ $catSUBSUB_sel['cat_id'] }}" {{ ($catSUBSUB_sel['cat_published'] == '0')? 'disabled':'' }} {{ (in_array($catSUBSUB_sel['cat_id'], $categoriesForAttribute))? 'selected':'' }}>- - {{ $catSUBSUB_sel['cat_name'] }}</option>
